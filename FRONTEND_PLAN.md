...
| **FE-4 Admin shell + CRM (P0)** ✅ | **เสร็จแล้ว (2026-07-22)** — AppShell (guarded route group) + **login + auth gate** (cookie session, RBAC matrix, `/api/v1/admin/auth/*`) · Dashboard (5 stat cards + pipeline funnel + activities + tasks) · **Leads index+detail** (master-detail, StatusChip, **status dropdown = `nextStatuses()` state machine เท่านั้น**, assign, notes/tasks timeline) · **Cancel-requirement dialog (บังคับ reason + cancelled_field, FR-CRM-07)**. admin = single-language ไทย, mock data. build + smoke test (auth gate/login/leads/404) เขียว | FR-SEC/CRM · Flow B |
