...
export function TrustGallery({ copy }: { copy: SectionCopy }) {
  const d = useDict();
  /* The wall used to be eight picsum.photos tiles captioned "รูปส่งมอบทรัพย์" —
     random stock images standing in as evidence of delivered deals. Now it
     renders only what the team uploaded, and the block disappears entirely
     when they have uploaded nothing. */
  const tiles = copy.itemsAny.filter((it) => it.img);
  ...
  // after the hooks, so the early exit cannot change their call order
  if (!copy.enabled || !tiles.length) return null;
    ...
    <section data-anim="1" style={{ padding: '72px 0 88px', overflow: 'hidden' }}>
      <div style={{ maxWidth: '1320px', margin: '0 auto', padding: '0 24px', textAlign: 'center' }}>
        ...
        <div style={{ margin: '18px auto 0', display: 'inline-flex', alignItems: 'center', gap: 9, height: 40, padding: '0 18px', borderRadius: 9999, background: 'var(--tint)', color: 'var(--accent)', fontSize: 14, fontWeight: 700 }}>
          <svg width="17" height="17" viewBox="0 0 24 24" fill="none" stroke="var(--accent)" strokeWidth="2.4" strokeLinecap="round" strokeLinejoin="round"><path d="M20 6L9 17l-5-5" /></svg>{pick(copy.sub, d.trust.happyClients)}
        </div>
      </div>
