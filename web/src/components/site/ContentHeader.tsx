...
/* ============================================================
   ...
   Ported verbatim from the identical <header> in About.dc.html /
   FAQ.dc.html / Contact.dc.html — sticky, 1320px, static 72px.
   Differs from ListingHeader only in which top-level link is
   marked active (via the `active` prop) and that โรงงาน is muted.
   Hover states reuse the globals.css helper classes.
   ============================================================ */
...
const chev = (rot: boolean, w = 11, stroke = 'currentColor', sw = '2.4') => (
  <svg width={w} height={w} viewBox="0 0 24 24" fill="none" stroke={stroke} strokeWidth={sw} style={{ transform: rot ? 'rotate(180deg)' : 'none', transition: 'transform .2s' }}>
    <path d="M6 9l6 6 6-6" />
  </svg>
);
...
export function ContentHeader({ active }: { active?: 'about' | 'faq' }) {
  ...
  const topLink = (label: string, href: string, isActive: boolean) => (
    <a
      className={isActive ? undefined : 'nav-link'}
      href={href}
      style={{ fontSize: 14, fontWeight: isActive ? 600 : 500, color: isActive ? 'var(--accent)' : 'var(--muted)' }}
    >
      {label}
    </a>
  );
  ...
  return (
    <>
        ...
        <div style={{ maxWidth: '1320px', margin: '0 auto', padding: '0 24px', height: '72px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          ...
          <nav style={{ display: 'flex', alignItems: 'center', gap: 26 }}>
            ...
            <div style={{ position: 'relative' }} onMouseEnter={() => setNavFactory(true)} onMouseLeave={() => setNavFactory(false)}>
              <div className="nav-link" style={{ display: 'flex', alignItems: 'center', gap: 4, fontSize: 14, fontWeight: 500, color: 'var(--muted)', cursor: 'pointer', padding: '4px 0' }}>
            ...
            {topLink('คำถามพบบ่อย', '/faq', active === 'faq')}
            {topLink('เกี่ยวกับเรา', '/about', active === 'about')}
            ...
            <div style={{ position: 'relative' }}>
              <div
                ...
                style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '7px 11px', border: '1px solid var(--border)', borderRadius: 9, fontSize: 13, fontWeight: 700, color: 'var(--text)', cursor: 'pointer' }}
              >
                {FLAG_SM[lang]}
                {LANG_LABEL[lang]}
      ...
      <div
        ...
        style={{ position: 'fixed', inset: 0, zIndex: 299, background: 'rgba(2,14,8,.55)', backdropFilter: 'blur(3px)', opacity: mobileOpen ? 1 : 0, pointerEvents: mobileOpen ? 'auto' : 'none', transition: 'opacity .3s' }}
      />
      {/* mobile drawer */}
      <div
        onClick={(e) => e.stopPropagation()}
