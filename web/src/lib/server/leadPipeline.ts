...
export async function advanceLead(
  ...
  try {
    ...
    /* ไม่ต่อท้ายด้วย ctx.reason อีกแล้ว — มันเป็นเหตุผลสำหรับ audit log เขียนว่า
       "deal cmt34u336000byg7a0pezi8ip won" ซึ่งอ่านแล้วไม่ได้อะไรเลยสำหรับคนดู
       lead รายละเอียดว่าดีลไหน ยอดเท่าไร ทรัพย์อะไร อยู่ในบรรทัดของเหตุการณ์นั้น
       เองแล้ว (ดู GET /api/leads/:id ที่ประกอบประวัติจากแถวจริง) */
    await db.leadNote.create({
      data: {
        leadId,
        userId: ctx.user.id,
        text: `สถานะเปลี่ยนเป็น "${STATUS_LABEL[target] ?? target}"`,
      },
