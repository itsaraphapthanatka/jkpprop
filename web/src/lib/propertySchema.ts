...
export type FieldKind =
  ...
  | 'price'
  | 'date' // date picker
  | 'person' // เลือกจากรายชื่อบัญชีในระบบ (ผู้ดูแลทรัพย์)
  | 'select'
  | 'multiselect'
  | 'boolean'
...
const WAREHOUSE_FIELDS: FieldDef[] = [
  ...
  /* ประกาศขายไม่ต้องมีช่องนี้ — ราคาต่อ ตร.ม. ของฝั่งขายคือ ราคาขาย ÷ พื้นที่
     ซึ่ง buildSpecs() คำนวณให้เองอยู่แล้ว การถามซ้ำมีแต่จะได้ตัวเลขที่ขัดกับ
     ราคาขายที่กรอกไว้ · ฝั่งเช่าเก็บไว้ เพราะบาทต่อ ตร.ม. ต่อเดือนเป็นตัวเลข
     ที่เจ้าของตั้งมาเอง ไม่ได้หารออกมาตรง ๆ เสมอไป */
  { key: 'price_per_sqm', label: 'ราคา / ตร.ม.', kind: 'number', unit: 'บาท/ตร.ม.', section: 'ราคา', note: 'เว้นว่างได้ — ระบบคำนวณจากราคากับพื้นที่ให้เอง', showWhen: WHEN_RENT },
  ...
  /* สไลด์ 46 · "ควรเชื่อม PIC ทุกช่องเองเพื่อรู้ว่าใครเป็นคนลง · ไปแสดงตอนสร้าง
     ประกาศ ออโต้" — เดิมเป็นช่องพิมพ์เปล่า ใครพิมพ์อะไรก็ได้ ตอนนี้เลือกจาก
     รายชื่อบัญชีจริง และเติมชื่อคนที่ล็อกอินอยู่ให้เองตอนสร้างใหม่ */
  { key: 'pic', label: 'ผู้ดูแลทรัพย์ (PIC)', kind: 'person', section: 'หมายเหตุ', internalOnly: true, note: 'เติมชื่อคุณให้อัตโนมัติตอนสร้างใหม่ — เปลี่ยนได้' },
