...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }): Promise<Metadata> {
  ...
  return { title: `${getDictionary(locale).titles.landRent} | JKP Property` };
}

export default function LandRentPage() {
  return (
    <ListingShell
      preset={{ breadcrumb: 'ที่ดินให้เช่า', listingMode: 'rent', typeSel: [propertyType('land').label], filterKey: 'land-rent' }}
    />
  );
}
