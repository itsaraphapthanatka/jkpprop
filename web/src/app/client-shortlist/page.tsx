...
const localeOf = (sp: Record<string, string | string[] | undefined>): Locale => {
  const raw = Array.isArray(sp.lang) ? sp.lang[0] : sp.lang;
  return isLocale(raw ?? '') ? (raw as Locale) : DEFAULT_LOCALE;
};

/* The tab title was Thai for every reader, including the customer the link was
   sent to in English. */
export async function generateMetadata({ searchParams }: { searchParams: Promise<Record<string, string | string[] | undefined>> }): Promise<Metadata> {
  const d = getDictionary(localeOf(await searchParams));
  return { title: `${d.clientShortlist.picked} | JKP Property`, robots: { index: false } };
}
...
export default async function ClientShortlistPage({ searchParams }: { searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  const locale = localeOf(await searchParams);

  /* The contact card named a consultant who does not exist and dialled a made-up
     number. The company's real details already live in one place — and so does
     the address, which the header had typed in as a Thai constant. */
